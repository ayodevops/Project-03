# Project 3 Documentation

![sudo-apt-install](/images/sudo-apt-install.PNG)

![todo-sudo-apt-install-npm](/images/todo-sudo-apt-install-npm.PNG)

![todo-npm-init](/images/todo-npm-init.PNG)

![npm-install-express](/images/npm-install-express.PNG)

![index.js](/images/index.js.PNG)

![install-dotenv](/images/install-dotenv.PNG)

![server-running-on-port-5000](/images/server-running-on-port-5000.PNG)

![welcome-to-express](/images/welcome-to-express.PNG)

![todo-routes](/images/todo-routes.PNG)

![mkdir-models](/images/mkdir-models.PNG)

![GET-request](/images/GET-request.PNG)

![POST-api](/images/POST-api.PNG)

![install-concurrently](/images/install-concurrently.PNG)

![install-nodemon](/images/install-nodemon.PNG)

![create-react-app](/images/create-react-app.PNG)

![localhost-3000](/images/localhost-3000.PNG)

![install-axios](/images/install-axios.PNG)

![my-todos](/images/my-todos.PNG)
